...
def make_intro(settings, using_mp=False):
    ...
    if not using_mp: print(f"Text Time - Quick: {round(quick_end_time - quick_start_time, 2)}")
    ...
    if not using_mp: print(f"Text Time - How Well: {round(how_well_end_time - how_well_start_time, 2)}")
    ...
    if not using_mp: print(f"Text Time - Topic: {round(topic_end_time - topic_start_time, 2)}")
    ...
    if not using_mp: print(f"Text Time - 3 Questions: {round(three_questions_end_time - three_questions_start_time, 2)}")
    ...
    if not using_mp: print(f"Text Time - 5 Seconds: {round(seconds_end_time - seconds_start_time, 2)}")
    ...
    if not using_mp: print(f"Background Time: {round(background_end_time - background_start_time, 2)}")
    
    
    # this will overlay the text on the background
    intro_comb_start_time = time.time()
    intro_video_combined = CompositeVideoClip([background_image, 
                                               intro_quick_text_combined, 
                                               intro_how_well_text_combined, 
                                               intro_topic_text_combined,
                                               intro_three_questions_text_combined,
                                               intro_seconds_text_combined], 
                                                size=settings['screen_size'])
    intro_video_audio_combined = intro_video_combined.set_audio(intro_audio)
    intro_comb_end_time = time.time()
    if not using_mp: print(f"Intro Combining Time: {round(intro_comb_end_time - intro_comb_start_time, 2)}")
    ...
    if settings['render_in']:
        ...
        if using_mp: print("Intro Starting Render, Edit Took: ", round(intro_end_time - intro_start_time, 2), " seconds")
        ...
        intro_video_audio_combined.write_videofile(
                                ...
                                logger="bar" if not using_mp else None
                            )
        end_time = time.time()
...
# main make intro function
def make_questions(settings, question_num, using_mp=False):
    ...
    title_text = title_text.set_position(
        lambda t: Animations.pop_in_pop_out_position(t, audio_title.duration, 1, audio_duration_total, width, height, settings['screen_size'], settings['quick_position'][0], settings['quick_position'][1], settings['question_title_pop_in_overshoot']))
    ...
    if not using_mp: print(f"Text Time - Title: {round(title_end_time - title_start_time, 2)}")
    ...
    if not settings['testing']:
        for i in range(1, len(question_raw_text) + 1):
            ...
            new_clip = TextClip(new_text, 
                                ...
                                interline=settings['question_text_interline'])
            new_clip = new_clip.set_duration(char_duration)
            clips.append(new_clip)
    ...
    if not using_mp: print(f"Text Time - Question: {round(question_text_end_time - question_text_start_time, 2)}")
    ...
    if not using_mp: print(f"Text Time - Timer: {round(timer_end_time - timer_start_time, 2)}")
    ...
    if not using_mp: print(f"Text Time - Answer: {round(answer_end_time - answer_start_time, 2)}")
    ...
    if not using_mp: print(f"Text Time - Answer Image: {round(answer_image_end_time - answer_image_start_time, 2)}")
    ...
    video_combined = CompositeVideoClip([background_image, 
                                            answer_image,
                                            title_text_combined,
    ...
    if not using_mp: print(f"Q{question_num} Combining Time: {round(comb_end_time - comb_start_time, 2)}")
    
    
    # debug frame with everything in it
    if settings['use_timer_frame']:
        # timer frame
        Helpers.show_frame(video_combined, audio_duration_total - settings['question_end_silence_duration'] - audio_answer.duration - 2, question_num)
    else:
        # regular answer frame
        Helpers.show_frame(video_combined, audio_duration_total - 1, question_num)
    ...
    if render_question:
        ...
        if using_mp: print(f"Q{question_num}    Starting Render, Edit Took: ", round(q_end_time - q_start_time, 2), " seconds")
...
def make_outro(settings, using_mp=False):
    ...
    if not using_mp: print(f"Text Time - Thanks: {round(thanks_end_time - thanks_start_time, 2)}")
    ...
    if not using_mp: print(f"Text Time - Subscribe: {round(subscribe_end_time - subscribe_start_time, 2)}")
    ...
    if not using_mp: print(f"Text Time - Comment: {round(comment_end_time - comment_start_time, 2)}")
    ...
    # this will overlay the text on the background
    outro_comb_start_time = time.time()
    outro_video_combined = CompositeVideoClip([background_image, 
                                               thanks_text_combined, 
                                               outro_topic_text_combined, 
                                               comment_text_combined,
                                               subscribe_rectangle,
                                               subscribe_text_combined,], size=settings['screen_size'])
    outro_video_audio_combined = outro_video_combined.set_audio(outro_audio)
    outro_comb_end_time = time.time()
    if not using_mp: print(f"Outro Combining Time: {round(outro_comb_end_time - outro_comb_start_time, 2)}")
    ...
    if settings['render_ou']:
        ...
        if using_mp: print(f"Outro Starting Render, Edit Took: ", round(outro_end_time - outro_start_time, 2), " seconds")
    ...
    return outro_video_audio_combined